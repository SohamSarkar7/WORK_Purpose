...
def load_scheme_master(scheme_master_path, fund_col="Fund Name"):
    ...
    df_master["_first_token"] = df_master[fund_col].apply(get_first_meaningful_token)
...
# How similar two "first tokens" (AMC names) need to be to be treated as
# the same fund house. 100 would mean exact match only (the original,
# over-strict behaviour); this is deliberately looser to tolerate OCR
# noise while still blocking genuinely different AMCs.
FIRST_TOKEN_SIMILARITY_THRESHOLD = 70


def _differs_by_at_most_one_char(a, b):
    """
    True if `a` and `b` can be made identical by a single character
    insertion, deletion, or substitution. Cheap and only intended for
    short strings (AMC first tokens), not full fund names.
    """
    if a == b:
        return True
    ...
    if len_a == len_b:
        # Same length: count substitutions needed.
        mismatches = sum(1 for x, y in zip(a, b) if x != y)
        return mismatches <= 1

    # Lengths differ by exactly 1: check if deleting one character from
    # the longer string produces the shorter one.
...
def first_tokens_compatible(token_a, token_b, threshold=FIRST_TOKEN_SIMILARITY_THRESHOLD):
    """
    Fuzzy-tolerant replacement for exact first-token equality.

    The original safety check required the first meaningful token/AMC name
    to match EXACTLY, to stop wildly wrong cross-AMC matches (e.g. a DSP
    fund being mapped to an SBI fund). In practice this was too strict:
    OCR noise in just the first word - "360" misread as "36O", "NIPPON"
    misread as "NIPON", a stray character - made two names from the SAME
    fund house look different and get rejected before scoring ever ran, so
    a correct match was never even considered. That's very likely the main
    cause of fund names not mapping to the Scheme Master correctly.

    This keeps the same protective goal (still blocks genuinely different
    AMC names) while tolerating small OCR noise in that first token:
    - a single-character edit (insertion/deletion/substitution) is always
      accepted, since that's the signature of one OCR misread and
      genuinely different AMC names essentially never differ by only one
      character
    - otherwise, a general fuzzy-ratio threshold, the same approach
      already used everywhere else in this file

    Called with an empty token on either side, this always returns True -
    matching the original behaviour, where the safety check only ever
    triggered when BOTH tokens were non-empty.
    """
    ...
    # Tolerates OCR merging/splitting a token, e.g. "360" vs "360ONE".
    if token_a.startswith(token_b) or token_b.startswith(token_a):
        return True

    if _differs_by_at_most_one_char(token_a, token_b):
        return True

    similarity = SequenceMatcher(None, token_a, token_b).ratio() * 100
...
def map_to_scheme_master(extracted_name, df_master, fund_col="Fund Name", fuzzy_threshold=98):
    ...
    if extracted_first_token:
        # Vectorized pre-filter: keep only master rows that are still
        # eligible under the (now fuzzy-tolerant) first-token safety
        # check. This is the same protective rule as before - a DSP fund
        # should never be scored against SBI funds - but computed once
        # per image via pandas instead of recomputing the first token and
        # looping through every master row from scratch for every image.
        # The expensive part (full fund-name comparison, just below) then
        # only runs on this much smaller, plausible subset.
        first_tokens = df_master["_first_token"]
        candidate_mask = (first_tokens == "") | first_tokens.apply(
            lambda master_token: first_tokens_compatible(extracted_first_token, master_token)
        )
        candidates_df = df_master[candidate_mask]
    else:
        candidates_df = df_master

    for master_name, master_norm, master_compact in zip(
        candidates_df[fund_col].values,
        candidates_df["_norm"].values,
        candidates_df["_compact"].values
    ):
...
def find_target_fund_line(target_fund_name, ocr_items):
    ...
    if best_line is not None and best_score >= 70:
        ...
        if (
            ...
            and not first_tokens_compatible(target_first_token, line_first_token)
        ):
            return None
...
def collect_images(root_folder):
    ...
    image_files = [
        ...
        and DEBUG_FOLDER.lower() not in str(p).lower()
...
def process_prc_folder_with_easyocr(
    ...
    """
    NOTE on parallel_workers / fuzzy_threshold:
    These two arguments exist only so this function accepts the same call
    signature that PRC_pipeline.py (the CLI backend) already uses. The
    original code did not accept them, which meant PRC_pipeline.py would
    crash with a TypeError the moment it was actually run. Processing
    itself is still single-threaded (parallel_workers is accepted but not
    used) so behaviour for the existing GUI is completely unchanged.
    fuzzy_threshold is forwarded into the scheme-master matching logic
    below, defaulting to the same 98% used before this fix.
    """
    root_folder = Path(root_folder)
    ...
    if not image_files:
        ...
        return {"status": "No image files found", "images_processed": 0}
    ...
    try:
        ...
        # A bad/locked/corrupt Scheme Master file used to crash the whole
        # batch before a single image was processed. Now we fall back to
        # "no master" so PRC detection still runs and you still get an
        # Excel file, just without fund-name mapping.
        print(f"[WARNING] Could not load Scheme master ({e}). Continuing without it.")
        df_master = pd.DataFrame(columns=[scheme_master_fund_col])
    ...
    if reader is None:
        ...
        return {"status": "EasyOCR unavailable", "images_processed": 0}
    ...
    final_output_path = str(output_excel)
    try:
        _write_excel(final_output_path)
    except Exception as e:
        # Most common cause: the output .xlsx is already open in Excel
        # (locked for writing). Previously this exception would bubble up
        # and every image already OCR'd in this run was lost with nothing
        # saved. Now we save to a timestamped fallback name instead so the
        # completed work is never thrown away.
        import time
        fallback_path = str(
            Path(output_excel).with_name(
                Path(output_excel).stem
                + f"_{time.strftime('%Y%m%d_%H%M%S')}"
                + Path(output_excel).suffix
            )
        )
        print(f"[WARNING] Could not save to '{final_output_path}' ({e}).")
        print(f"[WARNING] Saving results to '{fallback_path}' instead. "
              f"(Close the original file in Excel before the next run.)")
        _write_excel(fallback_path)
        final_output_path = fallback_path

    print("\nProcessing completed.")
    print(f"Excel saved as: {final_output_path}")

    if debug:
        print(f"Debug files saved in: {DEBUG_FOLDER}")

    matched_count = int((df_final["Final Status"] == "Matched").sum())

    return {
        "status": "Completed",
        "images_processed": len(image_files),
        "matched": matched_count,
        "not_matched": len(image_files) - matched_count,
        "output_excel": final_output_path
    }
